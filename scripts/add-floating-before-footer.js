...
// PASO 1: Eliminar TODOS los círculos flotantes existentes
const removePattern = /\s*{\/\*\s*C[íi]rculos flotantes[\s\S]*?<\/div>\s*(?=\s*<footer)/g;
content = content.replace(removePattern, '');

// PASO 2: Buscar el <footer> directamente y agregar los círculos JUSTO ANTES
const footerPattern = /(\s+<footer className="bg-\[#F7F7F7\])/;
...
if (!footerPattern.test(content)) {
    console.error('❌ No se encontró el <footer>');
    process.exit(1);
}
...
console.log('✅ Círculos flotantes agregados correctamente ANTES del <footer>');
