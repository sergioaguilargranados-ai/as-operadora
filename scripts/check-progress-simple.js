// check-progress-simple.js - Verificar progreso del scraping
...
async function checkProgress() {
    try {
        ...
        // Tours con precio
        const priceResult = await pool.query('SELECT COUNT(*) FROM megatravel_packages WHERE price_usd IS NOT NULL AND price_usd > 0');
        ...
        const lastUpdated = await pool.query(`
            SELECT mt_code, name, price_usd, updated_at
        ...
        lastUpdated.rows.forEach((tour, idx) => {
            ...
            console.log(`   ${idx + 1}. ${tour.mt_code}: $${tour.price_usd || 'N/A'} USD (${time})`);
        });
